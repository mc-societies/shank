...
public class Log4JInjector<T> implements MembersInjector<T> {
    ...
    Log4JInjector(Field field, Class clazz, Directory logFolder, LoggerContext context) {
       this(field, getName(clazz), logFolder, context);
    }

    Log4JInjector(Field field, String name, Directory logFolder, LoggerContext context) {
        this.field = field;
        LoggingHelper.addUniqueFileAppender(context, name, logFolder.getSubFile(name + ".log").toFile());
